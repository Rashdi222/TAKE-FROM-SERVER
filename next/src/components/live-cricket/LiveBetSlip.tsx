...
function resolveBetSlipSuspensionMessage(reason: string | null | undefined): string {
  const r = (reason || "").trim().toLowerCase();
  if (!r || r === "live_reprice" || r === "repricing") return "Prices are updating. Your bet will be available in a moment.";
  if (r === "rain_delay" || r === "rain_break") return "Play is delayed. Betting resumes when the match continues.";
  if (r === "third_umpire_review" || r === "drs_review") return "A review is in progress. Betting resumes after the decision.";
  if (r === "innings_break") return "Innings break in progress. Betting resumes shortly.";
  if (r === "tie_scenario_detected") return "Scores are level — result being confirmed.";
  if (r === "super_over_in_progress") return "Super Over in progress. Betting resumes shortly.";
  if (r === "dls_target_revision_in_progress") return "DLS target being revised. Betting resumes after confirmation.";
  if (r === "critical_data_missing") return "Live data is incomplete. Betting resumes when the feed is restored.";
...
function userFacingBetError(message: string | null | undefined) {
  ...
  return message || "Unable to place bet right now.";
}
...
export function LiveBetSlip({ store, quote, onClose }: LiveBetSlipProps) {
  ...
  const latestResolvedQuote = useMemo(() => {
    if (!quote) return null;
    return latestQuoteFromOdds(latestOdds, stateVersion) || findReplacementQuote(oddsById, quote, stateVersion) || quote;
  }, [quote, latestOdds, oddsById, stateVersion]);
  ...
  const slipSuspended = rowSuspended || Boolean(marketSuspension) || (suspended && !latestOdds);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isApiError(error)) {
        ...
        if (error.status === 401) {
          setWarning("Log in again to place a live bet.");
          return;
        }
      ...
      setWarning("Unable to place bet right now.");
    }
  };
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 max-h-[88vh] overflow-y-auto overscroll-contain rounded-t-[28px] border border-[var(--c-border)] bg-[var(--c-surface-1)] shadow-[var(--shadow-2)] lg:inset-y-0 lg:right-0 lg:left-auto lg:max-h-none lg:w-[380px] lg:rounded-none lg:border-l lg:border-t-0">
        ...
        <div className="space-y-4 p-4 pb-[calc(env(safe-area-inset-bottom,0px)+1rem)] sm:p-5 lg:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--c-text-faint)]">Live Bet Slip</div>
              <div className="mt-1 text-xl font-semibold text-[var(--c-text)]">{activeQuote.label}</div>
            </div>
          ...
          <Card variant="surface-1" className="p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--c-text-faint)]">
              ...
              <span>{activeQuote.marketKey.replace(/_/g, " ")}</span>
            </div>
          </Card>
          ...
          {slipSuspended ? (
            <WarningCard
              message={resolveBetSlipSuspensionMessage(effectiveSuspensionReason)}
            />
          ) : null}
          ...
          {placed ? (
            <Card variant="surface-2" className="p-5 text-center">
              ...
              <p className="mt-2 text-sm text-[var(--c-text-muted)]">The live quote was accepted and submitted successfully.</p>
