...
import { MasterMethodToggle } from "@/components/payments/MasterMethodToggle";
import type { PaymentMethod } from "@/lib/api";
import { formatDateTime } from "@/lib/format";
import { useMasterPaymentMethods } from "@/hooks/useMasterPayments";
...
export default function MasterPaymentMethodsPage() {
  const { data, isLoading } = useMasterPaymentMethods();
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-[var(--c-text)]">My Payment Methods</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--c-text-muted)]">
            These are the banking and wallet rails shown only to players assigned to your account. This desk controls the exact destination details they see during deposits and withdrawals.
          </p>
        </div>
        <Link href="/master/payments/methods/create">
          ...
          {methods.map((method) => (
            <Card key={method.id} variant="surface-2" className="p-6">
              ...
              <div className="mt-5 flex items-center justify-between gap-3">
                <Link href={`/master/payments/methods/${method.id}`} className="text-sm text-[var(--c-accent)] hover:text-[var(--c-text)]">
                  Update config
                </Link>
                <MasterMethodToggle method={method} />
              </div>
            </Card>
          ))}
        </div>
