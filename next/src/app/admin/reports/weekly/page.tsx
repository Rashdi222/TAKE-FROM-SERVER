...
import { useAccountCurrencies, useSuperAdminWeeklyReport } from "@/hooks/useSuperAdmin";

export default function WeeklyReportPage() {
  const [currencyFilter, setCurrencyFilter] = useState("");
  const { data: currencyData } = useAccountCurrencies();
  const { data, isLoading } = useSuperAdminWeeklyReport();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold text-[var(--c-text)]">Weekly Report</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--c-text-muted)]">Rolling seven-day platform aggregates from the backend analytics service.</p>
      </div>

      {isLoading ? (
        <p className="text-[var(--c-text-muted)]">Loading weekly report...</p>
      ) : (
        <div className="space-y-6">
          ...
          <Card variant="surface-2" className="p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                <h2 className="text-xl font-semibold text-[var(--c-text)]">Weekly Per-Currency Totals</h2>
                <p className="mt-1 text-sm text-[var(--c-text-muted)]">Focus the weekly currency table on one account currency if needed.</p>
              </div>
