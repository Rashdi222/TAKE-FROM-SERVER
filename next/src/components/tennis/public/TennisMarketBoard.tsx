...
export const TennisMarketBoard = memo(function TennisMarketBoard({ odds }: Props) {
  ...
  return (
    <section className="rounded-[1.4rem] border border-white/10 bg-[linear-gradient(180deg,#07111b_0%,#04080e_100%)] p-4 text-white sm:p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[11px] uppercase tracking-[0.28em] text-white/40">Live Markets</p>
          <h2 className="mt-1 text-lg font-semibold tracking-[-0.02em] sm:text-xl">Current tennis prices</h2>
        </div>
      ...
      {entries.length === 0 ? (
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-2 2xl:grid-cols-3">
          {entries.map(([market, rows]) => (
            <div key={market} className="overflow-hidden rounded-[1rem] border border-white/8 bg-white/[0.03]">
              ...
              <div className="grid grid-cols-[minmax(0,1fr)_88px] border-b border-white/8 bg-white/[0.02] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-white/45">
                <span>Selection</span>
                <span className="text-right">Odds</span>
              </div>
              <div className="divide-y divide-white/6">
                {rows.map((odd, index) => (
                  <div key={`${odd.market_key}-${odd.selection_key}-${index}`} className="grid grid-cols-[minmax(0,1fr)_88px] items-center gap-2 px-3 py-2">
                    <div className="min-w-0">
                      <div className="truncate text-sm text-white/90">
                        {formatTennisSelectionLabel(odd.selection_name || odd.selection_key || "Selection")}
                      </div>
                      <div className="truncate text-[10px] uppercase tracking-[0.12em] text-white/45">
                        {odd.line ? `Line ${odd.line}` : formatTennisSelectionLabel(odd.scope || "match")}
                      </div>
                    </div>
                    <div className="rounded-lg border border-emerald-400/20 bg-emerald-500/10 px-2 py-1.5 text-center font-mono text-sm font-semibold text-emerald-200">
