...
import {
  ...
  useRejectDeposit,
  useRejectWithdrawal,
  useSuperAdminPaymentApprovals,
  useSuperAdminPaymentApprovalSummary,
} from "@/hooks/useSuperAdmin";

export default function SuperAdminPaymentApprovalsPage() {
  const { data, isLoading, isError } = useSuperAdminPaymentApprovals();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold text-[var(--c-text)]">Approval Desk</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--c-text-muted)]">
          Review pending manual deposits and withdrawals for self-service players. Receipt evidence is loaded through an authenticated backend fetch, not a public file URL.
        </p>
      </div>
