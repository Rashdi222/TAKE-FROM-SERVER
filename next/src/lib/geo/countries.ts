...
const COUNTRIES_BY_DIAL_CODE = new Map<string, CountryOption[]>();
for (const country of COUNTRIES) {
  const existing = COUNTRIES_BY_DIAL_CODE.get(country.dialCode) ?? [];
  existing.push(country);
  COUNTRIES_BY_DIAL_CODE.set(country.dialCode, existing);
}

const SORTED_DIAL_CODES = Array.from(COUNTRIES_BY_DIAL_CODE.keys()).sort((a, b) => b.length - a.length);
...
export function inferCountryFromPhone(phoneNumber?: string | null) {
  if (!phoneNumber) return null;
  const sanitized = sanitizePhoneNumberInput(phoneNumber);
  if (!sanitized.startsWith("+")) return null;

  for (const dialCode of SORTED_DIAL_CODES) {
    if (sanitized.startsWith(dialCode)) {
      return COUNTRIES_BY_DIAL_CODE.get(dialCode)?.[0] ?? null;
    }
  }
...
export function syncPhoneWithCountry(
  phoneNumber: string,
  ...
  if (!nextCountry) {
    return sanitizePhoneNumberInput(phoneNumber);
  }
  ...
  if (previousCountry && sanitized.startsWith(previousCountry.dialCode)) {
    return `${nextCountry.dialCode}${sanitized.slice(previousCountry.dialCode.length)}`;
  }

  if (!sanitized.startsWith("+")) {
    const localDigits = sanitized.replace(/^0+/, "");
    return `${nextCountry.dialCode}${localDigits}`;
  }
