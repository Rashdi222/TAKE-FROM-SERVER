defmodule Back.AI.CricketOddsAutomation do
  @moduledoc false
  ...
  def run_for_feed(%CompetitionFeed{sport: "cricket"} = feed, kind, match_ids, opts)
  ...
  defp inplay_state_hash(match) do
    payload = %{
      status: match.status,
      score: match.score,
      total_runs: get_in(match.raw_data || %{}, ["result", "total_runs"]),
      updated_score: get_in(match.raw_data || %{}, ["scores"])
    }
  ...
  defp empty_phase_summary do
    %{success: 0, failure: 0, skipped: 0}
  end

  defp automation_model(feed) do
    (feed.config || %{})["automation_model"]
  end
